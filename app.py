...
BIST100_SYMBOLS = [
    ...
    "THYAO.IS","TKFEN.IS","TOASO.IS","TSKB.IS","TTKOM.IS","TTRAK.IS","TUPRS.IS","TUREX.IS","TURSG.IS","ULKER.IS",
    "VAKBN.IS","YKBNK.IS","ZOREN.IS","ZOREL.IS"
]
...
def generate_signals(df, rsi_lower, rsi_upper, min_expected_increase):
    ...
    df['Buy_Signal'] = ((df['Close'] > df['SMA_50']) & (df['EMA_50'] > df['EMA_200']) &
                        (df['MACD_Line'] > df['MACD_Signal']) & (df['MACD_Line'] > 0) &
                        (df['%K'] > df['%D']) & (df['%K'] > 20) &
                        (df['RSI'] > rsi_lower) & (df['RSI'] < rsi_upper))
    df['Sell_Signal'] = ((df['Close'] < df['SMA_50']) & (df['EMA_50'] < df['EMA_200']) &
                         (df['MACD_Line'] < df['MACD_Signal']) & (df['MACD_Line'] < 0) & (df['%K'] < df['%D']) &
                         (df['RSI'] > 70))
...
def process_stock(symbol, interval, rsi_lower, rsi_upper, min_expected_increase):
    try:
        ...
        # GEvşetilmiş koşul: sadece son gün sinyali
